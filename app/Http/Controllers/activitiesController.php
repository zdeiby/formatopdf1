...
class activitiesController extends Controller
{
    ...
    foreach ($datosBD as $value) {
      ...
      <td><button class="btn btn-danger" type="button" style="color:white !important;"  onclick="eliminar('.$value->id.',\''.$value->foto.'\')">Eliminar</button></td>
    </tr>';
    }
    ...
    public function guardarimagen(Request $request){
      ...
      $url =$request->input('url');
        ...
        $rutaImagen = storage_path('app/public/temp/' . $nombreImagen);
        $image = \Image::make($rutaImagen);
        $image->save($rutaImagen, 15); // Calidad de compresión del 75%
    ...
    public function guardarimagenactualizar(Request $request){
      ...
      $rutaImagen = storage_path('app/public/temp/' . $identificadorImagen . '.jpg');
        ...
        $nuevaruta=storage_path('app/public/temp/' . $nombreImagen);
        $image = \Image::make($nuevaruta);
       $image->save($nuevaruta, 15); // Calidad de compresión del 75%

        $datosBD = $modelo->editarActivities($id,$input1,$input2,$identificadorUnico);


        // Retornar la URL de la imagen
        return 'Imagen cargada exitosamente. Ruta: ' . $nuevaruta . $input1 .'  ' .$input2.   $url;

    }

    public function eliminar(Request $request){
      $id=$request->input('id');
      $identificadorUnico=$request->input('identUnico');
      $rutaImagen = storage_path('app/public/temp/' . $identificadorUnico . '.jpg');
      $modelo = new ActivitiesModel();
      $modelo->eliminar($id);
      if (file_exists($rutaImagen)) {
        // Eliminar la imagen
        ...
        return response()->json(['message' => 'Imagen eliminada correctamente']);        } else {
            // La imagen no existe
